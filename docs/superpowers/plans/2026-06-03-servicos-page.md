# Página Serviços — Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Criar a página `/servicos` com hero 50/50, 3 cards com vídeo mudo no fundo e modal fullscreen por serviço com fundo colorido.

**Architecture:** Single page `app/(public)/servicos/page.tsx` com `'use client'`. Dados estáticos no topo do arquivo. Modal controlado por `useState<string | null>` com o `id` do serviço ativo. Vídeos como placeholder (src local `/videos/*.mp4`) — arquivos reais adicionados depois.

**Tech Stack:** Next.js 14 App Router, TypeScript strict, Tailwind v4, framer-motion (`AnimatePresence`, `motion.div`), fonte Aileron.
...
| Ação | Arquivo |
|---|---|
| Criar | `app/(public)/servicos/page.tsx` |

Nenhum componente externo novo — tudo inline no page. O layout já injeta `<Nav />` e `<Footer />` via `app/(public)/layout.tsx`.

---

### Task 1: Estrutura base + dados estáticos

**Files:**
- Create: `app/(public)/servicos/page.tsx`

- [ ] **Step 1: Criar o arquivo com tipo ServiceData e array `services`**
...
- [ ] **Step 2: Verificar que a rota `/servicos` renderiza sem erro**

```bash
cd /home/brunotsilva/code/ai-active/Sites/brinde/brinde-2026 && pnpm dev
```

Abrir `http://localhost:3000/servicos` — deve mostrar "TODO" sem erros de TypeScript.

- [ ] **Step 3: Commit**

```bash
git add app/\(public\)/servicos/page.tsx
git commit -m "feat: página serviços — estrutura base e dados estáticos"
```

---

### Task 2: Seção Hero (pg 22)

**Files:**
- Modify: `app/(public)/servicos/page.tsx`

- [ ] **Step 1: Substituir o `<p>TODO</p>` pelo hero**

Substituir o conteúdo do `return` em `ServicosPage`:
...
`http://localhost:3000/servicos` — hero com título enorme esq + texto dir, fundo branco.

- [ ] **Step 3: Commit**

```bash
git add app/\(public\)/servicos/page.tsx
git commit -m "feat: serviços — seção hero 50/50"
```

---

### Task 3: Cards dos 3 Serviços (pg 23)

**Files:**
- Modify: `app/(public)/servicos/page.tsx`

- [ ] **Step 1: Adicionar componente `ServiceCard` (inline, acima de `ServicosPage`)**

Adicionar antes do `export default function ServicosPage`:
...
- [ ] **Step 2: Adicionar seção de cards no `return` de `ServicosPage`**

Substituir o comentário `{/* cards e modal vêm nas próximas tasks */}` por:
...
`http://localhost:3000/servicos` — 3 cards em coluna (mobile) ou 3 colunas (desktop). Vídeos não carregam ainda (placeholder path) — fundo escuro por padrão. Tags coloridas visíveis. Click não faz nada ainda.

- [ ] **Step 4: Commit**

```bash
git add app/\(public\)/servicos/page.tsx
git commit -m "feat: serviços — cards dos 3 serviços com vídeo fundo"
```

---

### Task 4: Modal fullscreen por serviço (pgs 24–26)

**Files:**
- Modify: `app/(public)/servicos/page.tsx`

- [ ] **Step 1: Adicionar componente `ServiceModal` (inline, acima de `ServiceCard`)**
...
- [ ] **Step 2: Adicionar `AnimatePresence` + render do modal no `return` de `ServicosPage`**

Substituir o comentário `{/* modal vem na próxima task */}` por:
...
`http://localhost:3000/servicos` — clicar em qualquer card deve abrir modal fullscreen com:
- Fundo na cor do serviço (verde / laranja / lilás)
- Botão X no canto superior direito
- Título grande + textos + listas
- ESC fecha o modal
...
```bash
git add app/\(public\)/servicos/page.tsx
git commit -m "feat: serviços — modal fullscreen por serviço com AnimatePresence"
```

---

### Task 5: Scroll lock no body quando modal aberto

**Files:**
- Modify: `app/(public)/servicos/page.tsx`

- [ ] **Step 1: Adicionar `useEffect` de scroll lock em `ServicosPage`**

Dentro de `ServicosPage`, logo após o `useState`:
...
Abrir modal → scroll da página de fundo deve travar. Fechar → scroll volta.

- [ ] **Step 3: Commit**

```bash
git add app/\(public\)/servicos/page.tsx
git commit -m "feat: serviços — scroll lock no body quando modal aberto"
```

---

### Task 6: Entrada animada dos cards com scroll

**Files:**
- Modify: `app/(public)/servicos/page.tsx`

- [ ] **Step 1: Substituir `<div>` raiz do `ServiceCard` por `motion.div` com viewport animation**

No componente `ServiceCard`, substituir o `<div onClick={onClick} ...>` externo por:
...
Ao rolar até a seção de cards, cada card deve aparecer com fade+slide-up suave.

- [ ] **Step 3: Commit**

```bash
git add app/\(public\)/servicos/page.tsx
git commit -m "feat: serviços — entrada animada dos cards com whileInView"
```
