...
export function Nav() {
  ...
  const textCls = dark ? 'text-[#050a30]' : 'text-white';
  const hoverCls = dark ? 'hover:text-[#050a30]/70' : 'hover:text-white/70';

  return (
    <nav className="fixed top-0 left-0 right-0 z-50 px-6 py-4 flex items-center justify-between transition-colors duration-300">
      <Link href="/" className="grid shrink-0" aria-label="Brinde">
        {/* eslint-disable @next/next/no-img-element */}
        <img
          src={LOGO_ESCURA}
          alt=""
          aria-hidden
          className={`h-12 md:h-14 w-auto col-start-1 row-start-1 transition-opacity duration-500 ${dark ? 'opacity-100' : 'opacity-0'}`}
        />
        <img
          src={LOGO_CLARA}
          alt=""
          aria-hidden
          className={`h-12 md:h-14 w-auto col-start-1 row-start-1 transition-opacity duration-500 ${dark ? 'opacity-0' : 'opacity-100'}`}
        />
